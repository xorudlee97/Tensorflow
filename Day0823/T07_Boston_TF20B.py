...
Node_number = 32
def Build_Model(X, train_num, test_num):
    Model_Layer = tf.layers.dense(X, train_num, activation=tf.sigmoid)
    ...
    Model_Layer = tf.layers.dense(Model_Layer, Node_number, activation=tf.nn.relu)
    Model_Layer = tf.layers.dense(Model_Layer, Node_number, activation=tf.nn.relu)
    Model_Layer = tf.layers.dense(Model_Layer, test_num, activation=tf.nn.relu)
    return Model_Layer;
...
model = Build_Model(X,train_num, test_num)
...
with tf.Session() as sess:
    ...
    prediction = sess.run(model, feed_dict={X:x_test})
